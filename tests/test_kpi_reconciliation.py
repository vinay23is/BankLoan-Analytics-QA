...
def loan_df():
    ...
    return pd.DataFrame({
        ...
        'int_rate':     [0.125] * 60 + [0.180] * 40,   # decimal format
...
def test_avg_interest_rate_is_percentage(loan_df):
    # int_rate stored as decimal; KPI should be expressed as %
    kpis = compute_kpis_pandas(loan_df)
    assert kpis['avg_interest_rate_pct'] == pytest.approx(14.7, abs=0.1)
...
def test_pandas_and_sql_kpis_agree(loan_df):
    ...
    assert len(failures) == 0, (
        "KPI mismatches detected:\n"
        + "\n".join(
            f"  {r.kpi_name}: pandas={r.pandas_value}, sql={r.sql_value}, diff={r.difference}"
            for r in failures
        )
    )
...
def test_reconciler_catches_deliberate_discrepancy():
    ...
    sql_kpis    = {'good_loan_rate_pct': 62.5}   # simulates a BI layer error
    results = reconcile(pandas_kpis, sql_kpis, tolerance=0.01)
...
def test_reconciler_passes_within_tolerance():
    pandas_kpis = {'avg_interest_rate_pct': 14.8500}
    sql_kpis    = {'avg_interest_rate_pct': 14.8502}   # floating-point rounding
    results = reconcile(pandas_kpis, sql_kpis, tolerance=0.01)
